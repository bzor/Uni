...
float4 _CameraViewTopLeftCorner[LKG_VIEWCOUNT];
float4x4 _CameraViewProjections[LKG_VIEWCOUNT]; // This is different from UNITY_MATRIX_VP (platform-agnostic projection matrix is used). Handle both non-XR and XR modes.
...
float4 _CameraViewXExtent[LKG_VIEWCOUNT];
float4 _CameraViewYExtent[LKG_VIEWCOUNT];
float4 _CameraViewZExtent[LKG_VIEWCOUNT];
