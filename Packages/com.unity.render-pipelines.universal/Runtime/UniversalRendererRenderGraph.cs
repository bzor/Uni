...
namespace UnityEngine.Rendering.Universal
{
    ...
    public sealed partial class UniversalRenderer
    {
        ...
        internal override void OnRecordRenderGraph(RenderGraph renderGraph, ScriptableRenderContext context)
        {
            ...
            // lkg edit
            BeginRenderGraphLKGRendering(renderGraph);
            ...
            // lkg edit
            EndRenderGraphLKGRendering(renderGraph);
        }
        ...
        private void OnAfterRendering(RenderGraph renderGraph)
        {
            ...
            // // lkg copy depth
            // if (cameraData.targetTexture != null && cameraData.targetTexture.volumeDepth == LookingGlassSettings.VIEWCOUNT)
            // {
            //     var multiview = cameraData.camera.GetComponentInParent<MultiviewContainer>();
            //     if (multiview.data != null && multiview.data.depthRt != null)
            //     {
            //         if (lkgDepthRTHandle == null || multiview.data.refreshDepthHandle)
            //         {
            //             lkgDepthRTHandle?.Release();
            //             lkgDepthRTHandle = RTHandles.Alloc(multiview.data.depthRt);
            //             multiview.data.refreshDepthHandle = false;
            //         }
            //         var lkgDepthTextureHandle = renderGraph.ImportTexture(lkgDepthRTHandle);
            //         m_LKGCopyDepthPass.CopyToDepth = true;
            //         m_LKGCopyDepthPass.MssaSamples = 1;
            //         m_LKGCopyDepthPass.Render(renderGraph, frameData, lkgDepthTextureHandle, resourceData.cameraDepthTexture, false, "LKG Depth Copy");
            //     }
            // }
