        ...
        static void RenderCameraStack(ScriptableRenderContext context, Camera baseCamera, bool isLastBaseCamera)
        {
            ...
            foreach ((Camera _, XRPass xrPass) in xrLayout.GetActivePasses())
            {
                ...
                using (new CameraRenderingScope(context, baseCamera))
                {
                    ...
                    if (baseCamera.targetTexture != null && LookingGlassSettings.IsViewCountOrHalf(baseCamera.targetTexture.volumeDepth))
                    {
                        cameraXRSettings.viewTotal = (uint)baseCamera.targetTexture.volumeDepth;
                        cameraXRSettings.viewCount = (uint)baseCamera.targetTexture.volumeDepth;
                    }
