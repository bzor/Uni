        ...
        public void RenderDoFBokeh(RenderGraph renderGraph, UniversalResourceData resourceData, UniversalCameraData cameraData, in TextureHandle source, in TextureHandle destination, ref Material dofMaterial)
        {
            ...
            using (var builder = renderGraph.AddUnsafePass<DoFBokehPassData>("Depth of Field - Bokeh", out var passData))
            {
                ...
                if (cameraData.targetTexture != null && LookingGlassSettings.IsViewCountOrHalf(cameraData.targetTexture.volumeDepth))
                {
                    ...
                    if (multiview.dofSettings.autoSetFocus)
                        P = multiview.RelativeDist * multiview.Size;
                    if (multiview.dofSettings.overrideSettings)
                    {
                        F = multiview.dofSettings.focalLength / 1000f;
                        A = multiview.dofSettings.focalLength / multiview.dofSettings.aperture;
                    }
                    multiplier = 2f;
                }
                ...
                float maxRadius = GetMaxBokehRadiusInPixels(m_Descriptor.height * multiplier);
