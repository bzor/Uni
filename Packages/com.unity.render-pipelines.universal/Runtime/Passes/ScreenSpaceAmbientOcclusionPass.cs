...
namespace UnityEngine.Rendering.Universal
{
    ...
    internal class ScreenSpaceAmbientOcclusionPass : ScriptableRenderPass
    {
        ...
        private Vector4[] m_CameraTopLeftCorner = new Vector4[LookingGlassSettings.VIEWCOUNTn];
        private Vector4[] m_CameraXExtent = new Vector4[LookingGlassSettings.VIEWCOUNTn];
        private Vector4[] m_CameraYExtent = new Vector4[LookingGlassSettings.VIEWCOUNTn];
        private Vector4[] m_CameraZExtent = new Vector4[LookingGlassSettings.VIEWCOUNTn];
        private RTHandle[] m_SSAOTextures = new RTHandle[4];
        private BlurTypes m_BlurType = BlurTypes.Bilateral;

        // lkg edit
        // private Matrix4x4[] m_CameraViewProjections = new Matrix4x4[2];
        private Matrix4x4[] m_CameraViewProjections = new Matrix4x4[LookingGlassSettings.VIEWCOUNTn];
        ...
        private void SetupKeywordsAndParameters(ref ScreenSpaceAmbientOcclusionSettings settings, ref UniversalCameraData cameraData)
        {
            ...
            MultiviewContainer multiview = null;
            if (cameraData.targetTexture != null && LookingGlassSettings.IsViewCountOrHalf(cameraData.targetTexture.volumeDepth))
            {
                multiview = cameraData.camera.GetComponentInParent<MultiviewContainer>();
                eyeCount = multiview.data.ViewCount;
                isMultiview = true;
            }
            ...
            for (int eyeIndex = 0; eyeIndex < eyeCount; eyeIndex++)
            {
                ...
                if (isMultiview)
                {
                    view = XRBuiltinShaderConstants.s_viewMatrix[eyeIndex];
                    proj = XRBuiltinShaderConstants.s_cameraProjMatrix[eyeIndex];
                }
