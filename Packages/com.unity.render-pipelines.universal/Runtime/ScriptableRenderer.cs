...
namespace UnityEngine.Rendering.Universal
{
    ...
    public abstract partial class ScriptableRenderer : IDisposable
    {
        private static partial class Profiling
        {
            ...
            // lkg edit
            internal static readonly ProfilingSampler beginLKGRendering = new ProfilingSampler($"Begin LKG Rendering");
            internal static readonly ProfilingSampler endLKGRendering = new ProfilingSampler($"End LKG Rendering");
        ...
        internal static void SetCameraMatrices(RasterCommandBuffer cmd, UniversalCameraData cameraData, bool setInverseMatrices, bool isTargetFlipped)
        {
            ...
            XRBuiltinShaderConstants.LookingGlassUpdate(cmd);
        ...
        internal void ProcessVFXCameraCommand(RenderGraph renderGraph)
        {
                       ...
                       Profiling.vfxProcessCamera))
            {
                ...
                if (cameraData.camera.targetTexture != null && LookingGlassSettings.IsViewCountOrHalf(cameraData.camera.targetTexture.volumeDepth))
                {
                    passData.cameraXRSettings.viewTotal = (uint)cameraData.camera.targetTexture.volumeDepth;
                    passData.cameraXRSettings.viewCount = (uint)cameraData.camera.targetTexture.volumeDepth;
                }
                ...
                builder.SetRenderFunc((VFXProcessCameraPassData data, UnsafeGraphContext context) =>
                {
                    ...
                    if (LookingGlassSettings.IsViewCountOrHalf((int)data.cameraXRSettings.viewCount))
                    {
                        context.cmd.EnableKeyword(SinglepassKeywords.UNITY_STEREO_INSTANCING_ENABLED);
                        context.cmd.SetInstanceMultiplier((uint)data.cameraXRSettings.viewCount);
                    }
                    ...
                    if (LookingGlassSettings.IsViewCountOrHalf((int)data.cameraXRSettings.viewCount))
                    {
                        context.cmd.DisableKeyword(SinglepassKeywords.UNITY_STEREO_INSTANCING_ENABLED);
                        context.cmd.SetInstanceMultiplier(1u);
                    }
                });
        ...
        Matrix4x4[] lkgViewMats = new Matrix4x4[LookingGlassSettings.VIEWCOUNTn];
        Matrix4x4[] lkgProjMats = new Matrix4x4[LookingGlassSettings.VIEWCOUNTn];

        private class BeginLKGPassData
        {
            internal UniversalCameraData cameraData;
            internal int viewCount;
        };

        internal void BeginRenderGraphLKGRendering(RenderGraph renderGraph)
        {
            UniversalCameraData cameraData = frameData.Get<UniversalCameraData>();
            if (cameraData.targetTexture == null || !LookingGlassSettings.IsViewCountOrHalf(cameraData.targetTexture.volumeDepth))
                return;

            var multiview = cameraData.camera.GetComponentInParent<MultiviewContainer>();
            if (multiview == null)
                return;

            for (int i = 0; i < LookingGlassSettings.VIEWCOUNTn; i++)
            {
                var normalizedView = (float)i / (multiview.data.ViewCount - 1);
                multiview.ResetCamera(normalizedView);
                cameraData.camera.ResetWorldToCameraMatrix();
                lkgViewMats[i] = cameraData.camera.worldToCameraMatrix;
                lkgProjMats[i] = cameraData.camera.projectionMatrix;
            }
            multiview.ResetCamera();

            XRBuiltinShaderConstants.LookingGlassMatrixUpdate(ref lkgViewMats, ref lkgProjMats);

            using (var builder = renderGraph.AddRasterRenderPass<BeginLKGPassData>("BeginLKGRendering", out var passData,
                Profiling.beginLKGRendering))
            {
                passData.cameraData = cameraData;
                passData.viewCount = multiview.data.ViewCount;

                builder.AllowPassCulling(false);
                builder.AllowGlobalStateModification(true);

                builder.SetRenderFunc((BeginLKGPassData data, RasterGraphContext context) =>
                {
                    context.cmd.EnableKeyword(SinglepassKeywords.UNITY_STEREO_INSTANCING_ENABLED);
                    context.cmd.SetInstanceMultiplier((uint)data.viewCount);

                    XRBuiltinShaderConstants.LookingGlassUpdate(context.cmd);
                });
            }
        }

        private class EndLKGPassData
        {
            public UniversalCameraData cameraData;
            public MultiviewData multiviewData;
            public Mesh fullscreenQuad;
            public Matrix4x4 matOrtho;
            public TextureHandle cameraDepthHandle;
        };

        Mesh fullscreenQuad;
        Matrix4x4 matOrtho = Matrix4x4.Ortho(0, 1, 0, 1, -1, 1);

        internal void EndRenderGraphLKGRendering(RenderGraph renderGraph)
        {
            UniversalCameraData cameraData = frameData.Get<UniversalCameraData>();
            if (cameraData.targetTexture == null || !LookingGlassSettings.IsViewCountOrHalf(cameraData.targetTexture.volumeDepth))
                return;
            ...
            multiview.data.ApplyCalibration();

            var resourceData = frameData.Get<UniversalResourceData>();

            // silly to put this here but whatever
            if (fullscreenQuad == null)
            {
                ...
                fullscreenQuad.triangles = new int[] { 0, 1, 2, 2, 1, 3 };
            }

            using (var builder = renderGraph.AddUnsafePass<EndLKGPassData>("EndLKGRendering", out var passData,
                Profiling.endLKGRendering))
            {
                passData.cameraData = cameraData;
                passData.multiviewData = multiview.data;

                passData.fullscreenQuad = fullscreenQuad;
                passData.matOrtho = matOrtho;
                ...
                builder.SetRenderFunc((EndLKGPassData data, UnsafeGraphContext context) =>
                {
                    #if UNITY_EDITOR
                    if (!MultiviewData.texturesGenerated)
                    {
                        context.cmd.DisableKeyword(SinglepassKeywords.UNITY_STEREO_INSTANCING_ENABLED);
                        context.cmd.SetInstanceMultiplier(1);
                        return;
                    }
                    #endif
                    ...
                    // prepare for full screen blitting
                    context.cmd.SetViewProjectionMatrices(
                        Matrix4x4.identity, data.matOrtho
                    );

                    if (data.multiviewData.genViewsOn)
                    {
                        // gen views compute
                        context.cmd.SetComputeTextureParam(data.multiviewData.genViewsCompute, 0, "DepthSource", data.cameraDepthHandle);
                        context.cmd.DispatchCompute(
                            data.multiviewData.genViewsCompute,
                            0,
                            1,
                            data.multiviewData.colorRt.height / 32,
                            data.multiviewData.colorRt.volumeDepth / 8
                        );

                        // make my fake views!
                        context.cmd.EnableKeyword(SinglepassKeywords.USE_CAMERA_DEPTH);
                        context.cmd.SetInstanceMultiplier((uint)data.multiviewData.ViewCount - 1);
                        context.cmd.SetRenderTarget(data.multiviewData.colorDepth2Array, data.multiviewData.colorRt2.depthBuffer, 0, CubemapFace.Unknown, -1);
                        context.cmd.DrawMesh(
                            data.fullscreenQuad,
                            Matrix4x4.identity,
                            data.multiviewData.genViewsMat, 0, 0
                        );
                    }

                    // dof
                    if (data.multiviewData.dofOn)
                    {
                        context.cmd.EnableKeyword(SinglepassKeywords.USE_CAMERA_DEPTH);
                        context.cmd.SetInstanceMultiplier((uint)data.multiviewData.ViewCount);
                        context.cmd.SetRenderTarget(data.multiviewData.colorRtFinal, 0, CubemapFace.Unknown, -1);
                        context.cmd.DrawMesh(
                            data.fullscreenQuad,
                            Matrix4x4.identity,
                            data.multiviewData.dofMat1, 0, 0
                        );
                        context.cmd.DisableKeyword(SinglepassKeywords.USE_CAMERA_DEPTH);

                        if (data.multiviewData.genViewsOn)
                        {
                            context.cmd.SetInstanceMultiplier((uint)data.multiviewData.ViewCount - 1);
                            context.cmd.SetRenderTarget(data.multiviewData.colorRt2Final, 0, CubemapFace.Unknown, -1);
                            context.cmd.DrawMesh(
                                data.fullscreenQuad,
                                Matrix4x4.identity,
                                data.multiviewData.dofMat2, 0, 0
                            );
                        }
                    }

#if UNITY_EDITOR
                    // if (data.multiviewData.isBlittingQuilt && data.multiviewData.quiltOutput != null)
                    if (data.multiviewData.quiltOutput != null)
                    {
                        context.cmd.SetInstanceMultiplier(1);
                        context.cmd.SetRenderTarget(data.multiviewData.quiltOutput);
                        context.cmd.DrawMesh(
                            data.fullscreenQuad,
                            Matrix4x4.identity,
                            data.multiviewData.quiltMat, 0, 0
                        );
                    }
#endif
                    ...
                    // draw lenticular
                    context.cmd.SetInstanceMultiplier(1);
                    context.cmd.SetRenderTarget(data.multiviewData.lenticularRt);
                    context.cmd.DrawMesh(
                        data.fullscreenQuad,
                        Matrix4x4.identity,
                        data.multiviewData.lenticularMat, 0, 0
                    );
                });
            }
        }
        ...
        public void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmdScope, profilingExecute))
            {
                ...
                using (new ProfilingScope(Profiling.setupCamera))
                {
                    ...
                    if (cameraData.camera.targetTexture != null && LookingGlassSettings.IsViewCountOrHalf(cameraData.camera.targetTexture.volumeDepth))
                    {
                        cameraXRSettings.viewTotal = (uint)cameraData.camera.targetTexture.volumeDepth;
                        cameraXRSettings.viewCount = (uint)cameraData.camera.targetTexture.volumeDepth;
                    }

                    if (cameraData.xr.enabled)
                        cameraData.xr.StartSinglePass(cmd);

                    if (LookingGlassSettings.IsViewCountOrHalf((int)cameraXRSettings.viewCount))
                    {
                        cmd.EnableKeyword(SinglepassKeywords.UNITY_STEREO_INSTANCING_ENABLED);
                        cmd.SetInstanceMultiplier(cameraXRSettings.viewCount);
                    }
                    ...
                    if (LookingGlassSettings.IsViewCountOrHalf((int)cameraXRSettings.viewCount))
                    {
                        cmd.DisableKeyword(SinglepassKeywords.UNITY_STEREO_INSTANCING_ENABLED);
